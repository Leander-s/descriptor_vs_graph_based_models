...
from dgllife.utils.featurizers import BaseAtomFeaturizer, BaseBondFeaturizer, ConcatFeaturizer, atom_type_one_hot, atom_degree_one_hot, \
...
class Meter(object):
    ...
    def accuracy(self):
        ...
        for task in range(n_tasks):
            ...
            task_y_true = y_true[:, task][task_w != 0]
            task_y_pred = y_pred[:, task][task_w != 0]
            correct = np.sum(task_y_true == task_y_pred)
            scores.append(correct/n_data)
        return scores
    ...
    def compute_metric(self, metric_name, reduction='mean'):
        ...
        list of float
            Metric value for each task
        """
        assert metric_name in ['roc_auc', 'l1', 'rmse', 'prc_auc', 'mae', 'r2', 'mcc', 'accuracy'], \
