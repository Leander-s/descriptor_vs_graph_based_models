...
def run_attentivefp(parameters: RunParameters) -> (pd.DataFrame, float):
    start = time.time()
    torch.backends.cudnn.enabled = True
    torch.backends.cudnn.benchmark = True
    set_random_seed(seed=43)
    task_type = parameters.task_type  # 'cla' or 'reg'
    print(f"AFP dataset {parameters.dataset}")
...
#    torch.cuda.set_device(eval(gpu_id))  # gpu device id
    ...
    hyper_space = dict(l2=hp.choice('l2', [0, 10 ** -8, 10 ** -6, 10 ** -4]),
                       lr=hp.choice(
                       ...
                       graph_feat_size=hp.choice(
                           'graph_feat_size',
                           [50, 100, 200, 300])
                       )

# get the df and generate graph, attention for graph generation for some
# bad smiles
...
# get the training, validation, and test sets
    ...
    def hyper_opt(hyper_paras):
        ...
        my_model = AttentiveFPPredictor(
            node_feat_size=AtomFeaturizer.feat_size('h'),
            edge_feat_size=BondFeaturizer.feat_size('e'),
            num_layers=hyper_paras['num_layers'],
            num_timesteps=hyper_paras['num_timesteps'],
            graph_feat_size=hyper_paras['graph_feat_size'],
            n_tasks=len(tasks),
            dropout=hyper_paras['dropout'])
        model_file_name = './saved_model/%s_%s_%s_%.6f_%s_%s_%s_%s.pth' % (
...
# regenerate the graphs
    ...
    for split in range(1, repetitions + 1):
        ...
        best_model = AttentiveFPPredictor(
            node_feat_size=AtomFeaturizer.feat_size('h'),
            edge_feat_size=BondFeaturizer.feat_size('e'),
