...
def run_gat(parameters: RunParameters) -> (pd.DataFrame, float):
    start = time.time()
...
# train_set, val_set, test_set = Subset(my_dataset, tr_indx), Subset(my_dataset, val_indx), Subset(my_dataset, te_indx)
    ...
    def hyper_opt(hyper_paras):
        ...
        for j in range(epochs):
            # training
            run_a_train_epoch(my_model, train_loader,
                              loss_func, optimizer, args)
...
# regenerate the graphs
    ...
    for split in range(1, repetitions + 1):
        ...
        best_model = GATPredictor(
            in_feats=AtomFeaturizer.feat_size('h'),
            hidden_feats=hidden_feats_ls[opt_res['gat_hidden_feats']],
