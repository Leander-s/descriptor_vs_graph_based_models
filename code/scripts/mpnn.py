...
def run_a_train_epoch(model, data_loader, loss_func, optimizer, args):
    ...
    for batch_id, batch_data in enumerate(data_loader):
        ...
        labels, masks, atom_feats, bond_feats = (
            labels.to(args['device']), masks.to(args['device']),
        ...
        labels = labels.cpu()
        masks = masks.cpu()
        atom_feats = atom_feats.cpu()
        bond_feats = bond_feats.cpu()
        loss = loss.cpu()
...
def run_mpnn(parameters: RunParameters) -> (pd.DataFrame, float):
    start = time.time()
...
# get the df and generate graph, attention for graph generation for some bad
# smiles
# my_df.iloc[:, 0:-1], except with 'group'

    my_dataset: MoleculeCSVDataset = csv_dataset.MoleculeCSVDataset(
        my_df.iloc[:, 0:-1], smiles_to_bigraph, AtomFeaturizer,
...
# get the training, validation, and test sets
    ...
    def hyper_opt(hyper_paras):
        ...
        my_model = MPNNPredictor(
            node_in_feats=AtomFeaturizer.feat_size('h'),
            edge_in_feats=BondFeaturizer.feat_size('e'),
            n_tasks=len(tasks),
            node_out_feats=hyper_paras['node_hidden_dim'],
            edge_hidden_feats=hyper_paras['edge_hidden_dim'],
            num_layer_set2set=hyper_paras['num_layer_set2set']
        )
        model_file_name = './saved_model/%s_%s_%s_%.6f_%s_%s_%s.pth' % (
        ...
        optimizer = torch.optim.Adam(
            my_model.parameters(), lr=hyper_paras['lr'],
...
# regenerate the graphs
    ...
    for split in range(1, repetitions + 1):
        ...
        if args['metric'] == 'roc_auc':
            ...
            while True:
                ...
                if np.any(data_tr[tasks].apply(all_one_zeros)) or \
                    ...
                    print("\ninvalid random seed {} due to one class presented"
                          " in the splitted {} sets..."
            ...
            # the training set was further splitted into the training set and
            # validation set
            data_tr, data_va = train_test_split(
        ...
        best_model = MPNNPredictor(
            node_in_feats=AtomFeaturizer.feat_size('h'),
            edge_in_feats=BondFeaturizer.feat_size('e'),
            n_tasks=len(tasks),
            node_out_feats=node_hidden_dim_ls[opt_res['node_hidden_dim']],
            edge_hidden_feats=edge_hidden_dim_ls[
                opt_res['edge_hidden_dim']
            ],
            num_layer_set2set=num_layer_set2set_ls[
        ...
        for j in range(epochs):
            ...
            print("task:{} repetition {:d}/{:d} epoch {:d}/{:d}, training {} "
                  ...
                  .format(
                      args['task'], split, repetitions, j + 1,
                      epochs, args['metric'], train_scores[args['metric']],
