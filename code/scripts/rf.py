...
def hyper_runing(task_args):
    (subtask, dataset, dataset_label, task_type, OPT_ITERS, space_,
     n_estimators_ls, max_depth_ls, min_samples_leaf_ls,
     max_features_ls) = task_args
    ...
    def hyper_opt(args):
        if task_type == "cla":
            model = RandomForestClassifier(
                **args, n_jobs=6, random_state=1, verbose=0,
    ...
    trials = Trials()
    best_results = fmin(
        hyper_opt, space_, algo=tpe.suggest,
    ...
    if task_type == "cla":
        best_model = RandomForestClassifier(
            ...
            min_samples_leaf=min_samples_leaf_ls[
    ...
    if task_type == 'cla':
        ...
        positives = data_te_y[data_te_y == 1].shape[0]
        negatives = data_te_y[data_te_y == 0].shape[0]
        n_over_p = negatives/positives if positives != 0 else "na"
        te_results = [
            dataset_label, subtask, 'te', num_fea, num_of_compounds,
            data_te_y[data_te_y == 1].shape[0],
            data_te_y[data_te_y == 0].shape[0],
            n_over_p, n_estimators_ls[best_results['n_estimators']],
            max_depth_ls[best_results['max_depth']],
            min_samples_leaf_ls[best_results['min_samples_leaf']],
            best_results['min_impurity_decrease'],
            max_features_ls[best_results['max_features']]]
...
def best_model_runing(args):
    ...
    if task_type == 'cla':
        while True:
            ...
            if (
                ...
                print("random seed used in repetition {} is {}".format(
                    split, seed))
                break
    else:
        training_data, data_te = train_test_split(
    ...
    if task_type == "cla":
        model = RandomForestClassifier(
            ...
            .iloc[0,]['max_depth'],
            min_samples_leaf=best_hyper[best_hyper.subtask == subtask]
            .iloc[0,]['min_samples_leaf'],
            max_features=best_hyper[best_hyper.subtask == subtask]
            .iloc[0,]['max_features'],
            min_impurity_decrease=best_hyper[best_hyper.subtask == subtask]
...
def run_rf(parameters: RunParameters) -> (pd.DataFrame, float):
    start = time.time()
    ...
    if task_type == 'cla':
        ...
        best_hyper = pd.DataFrame(
            pd_res, columns=[
                ...
                'max_features', 'rmse', 'r2', 'mae'])

    best_hyper.to_csv(
        f"./stat_res/hyperopts/{dataset_label}"
        "_moe_pubsub_rf_hyperopt_info.csv", index=0)
...
# 50 repetitions based on thr best hypers
    ...
    for subtask in tasks:
        ...
        args = [(split, task_type, sub_dataset, subtask, best_hyper,
                 dataset_label)
