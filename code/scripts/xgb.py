...
def hyper_runing(task_args):
    (subtask, dataset, dataset_label, task_type, OPT_ITERS, patience, space_,
     min_child_weight_ls, max_depth_ls, n_estimators_ls) = task_args
    cols = [subtask]
    ...
    pos_weight = (len(sub_dataset) -
                  sum(sub_dataset[subtask])) / sum(sub_dataset[subtask])
    ...
    def hyper_opt(args):
        ...
        if task_type == "cla":
            model = XGBClassifier(
                **args, n_jobs=6, random_state=1, seed=1,
                scale_pos_weight=pos_weight, eval_metric='auc',
                early_stopping_rounds=patience)
        else:
            model = XGBRegressor(
                **args, n_jobs=6, random_state=1, seed=1,
                eval_metric='rmse', early_stopping_rounds=patience)
    ...
    if task_type == "cla":
        best_model = XGBClassifier(
            ...
            min_child_weight=min_child_weight_ls[
                best_results['min_child_weight']],
            ...
            eval_metric='auc', early_stopping_rounds=patience)
    else:
        best_model = XGBRegressor(
            ...
            subsample=best_results['subsample'],
            colsample_bytree=best_results['colsample_bytree'],
            n_jobs=6, random_state=1, seed=1, eval_metric='rmse',
            early_stopping_rounds=patience)
    ...
    if task_type == 'cla':
        # training set
        tr_pred = best_model.predict_proba(
            data_tr_x)
        tr_results = [
            ...
            best_results['learning_rate'], best_results['gamma'],
            best_results['subsample'], best_results['colsample_bytree']]
        tr_results.extend(statistical(
            data_tr_y, np.argmax(tr_pred, axis=1), tr_pred[:, 1]))

        # validation set
        va_pred = best_model.predict_proba(
            data_va_x)
        va_results = [
        ...
        # test set
        te_pred = best_model.predict_proba(
            data_te_x)
        te_results = [
...
def best_model_runing(args):
    (split, patience, task_type, sub_dataset, subtask, best_hyper,
     dataset_label) = args
    seed = split
    if task_type == 'cla':
        while True:
    ...
    if task_type == "cla":
        model = XGBClassifier(
            n_estimators=best_hyper[best_hyper.subtask == subtask]
            .iloc[0,]['n_estimators'],
            max_depth=best_hyper[best_hyper.subtask == subtask]
            .iloc[0,]['max_depth'],
            min_child_weight=best_hyper[best_hyper.subtask == subtask]
            .iloc[0,]['min_child_weight'],
            learning_rate=best_hyper[best_hyper.subtask == subtask]
            .iloc[0,]['learning_rate'],
            gamma=best_hyper[best_hyper.subtask == subtask]
            .iloc[0,]['gamma'],
            subsample=best_hyper[best_hyper.subtask == subtask]
            .iloc[0,]['subsample'],
            colsample_bytree=best_hyper[best_hyper.subtask == subtask]
            .iloc[0,]['colsample_bytree'],
            n_jobs=6, random_state=1, seed=1, scale_pos_weight=pos_weight,
            eval_metric='auc', early_stopping_rounds=patience)
    else:
        model = XGBRegressor(
...
def run_xgb(parameters: RunParameters) -> (pd.DataFrame, float):
    start = time.time()
    warnings.filterwarnings("ignore")
    task_type = parameters.task_type  # 'reg' or 'cla'
    dataset_label = parameters.label  # dataset_label = 'esol'
    ...
    if task_type == 'cla':
        best_hyper = pd.DataFrame(
            pd_res, columns=[
                ...
                'negtives/postives', 'n_estimators', 'max_depth',
                'min_child_weight', 'learning_rate', 'gamma', 'subsample',
                'colsample_bytree', 'tn', 'fp', 'fn', 'tp', 'se', 'sp',
                'acc', 'mcc', 'auc_prc', 'auc_roc'])
    else:
        best_hyper = pd.DataFrame(
            pd_res, columns=[
                'dataset', 'subtask', 'set', 'num_of_retained_feature',
                'num_of_compounds', 'n_estimators', 'max_depth',
                'min_child_weight', 'learning_rate', 'gamma', 'subsample',
                'colsample_bytree', 'rmse', 'r2', 'mae'])

    best_hyper.to_csv(f"./stat_res/hyperopts/{dataset_label}"
                      "_moe_pubsub_xgb_hyperopt_info.csv", index=0)
...
# multi-tasks
        ...
        for subtask in tasks:
            ...
            tr_acc = (tr_acc + tr[tr['subtask'] == subtask][args['metric']]
                      .values)

            va = stat_res[stat_res['set'] == 'va']
            va_acc = (va_acc + va[va['subtask'] == subtask][args['metric']]
                      .values)

            te = stat_res[stat_res['set'] == 'te']
            te_acc = (te_acc + te[te['subtask'] == subtask][args['metric']]
                      .values)
