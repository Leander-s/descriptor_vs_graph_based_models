...
def isolate_tasks(dataset_name, amount):
    ...
    minimol_path = f"../minimol_datasets/{dataset_name}-minimol_descriptors.csv"
    moe_neutral_path = f"../moe-neutral_datasets/{dataset_name}_moe-neutral_descriptors.csv"
    ...
    for i in range(1, amount + 1):
        ...
        new_rdkit_path = f"../additional_datasets/rdkit/{dataset_name}{i}_rdkit_descriptors.csv"
        new_rdkit_frame.to_csv(new_rdkit_path, index=False)
        new_minimol_frame = isolate_task(minimol_frame, i, max_tasks)
        new_minimol_path = f"../additional_datasets/minimol/{dataset_name}{i}_minimol_descriptors.csv"
        new_minimol_frame.to_csv(new_minimol_path, index=False)
        new_moe_neutral_frame = isolate_task(moe_neutral_frame, i, max_tasks)
        new_moe_neutral_path = f"../additional_datasets/moe-neutral/{dataset_name}{i}_moe-neutral_descriptors.csv"
